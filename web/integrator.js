...
function renderState(state, latency) {
  ...
  const connected = state.device.connected, mqtt = state.mqtt || {}, mqttState = mqtt.sensor_state; const pill = $("dev-connection-pill"); pill.className = `connection-pill ${mqtt.enabled ? mqttState === "ONLINE" ? "online" : mqttState === "OFFLINE" ? "offline" : "" : connected ? "online" : state.simulation.enabled ? "" : "offline"}`;
  ...
  text("raw-distance", shown(state.raw.soil_moisture, "%")); text("raw-light", shown(state.raw.temperature, "°C")); text("raw-health", d.health_score == null ? "—" : `${d.health_score}/100`); text("raw-buzzer", d.buzzer_mode); text("raw-sequence", state.raw.sequence >= 0 ? state.raw.sequence : "—");
